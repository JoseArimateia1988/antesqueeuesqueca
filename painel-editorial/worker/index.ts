...
- Traga no máximo 5 sugestões. Se houver só 2 realmente boas, entregue 2.
...
- Considere literalmente o texto escrito por ela; não responda com uma lista genérica da lente.
- Use busca na web para encontrar destinos atuais e links diretos. Não invente URLs.
- Dê preferência à fonte original. Evite páginas agregadoras quando houver fonte primária.
- Explique por que cada item responde ao rumo desta busca, e não apenas por que ele pertence à lente.
...
LENTE ATUAL: ${lens}
CRITÉRIO: ${lensRules[lens]}

Retorne somente um objeto JSON entre as tags <RADAR_JSON> e </RADAR_JSON>, sem markdown e sem texto antes ou depois. Formato:
{"ideas":[{"type":"tipo curto","title":"nome real","description":"o que é em linguagem simples","reason":"por que entrou a partir do texto da Camis","connection":"${lens}","time":"tempo para explorar","url":"https://link-direto","verification":"o que foi verificado e o que ainda precisa de exploração manual"}]}`;

  const modeDescription = mode === "specific" ? "A Camis trouxe uma pista concreta; investigue a partir dela." : mode === "moment" ? "A busca parte do que a Camis está vivendo ou pensando; preserve as palavras e tensões que ela trouxe." : "A Camis quer que a IA abra caminhos dentro da lente, sem exigir um tema fechado.";
  const userMessage = `${modeDescription}\n\nO que ela escreveu: ${prompt || "nenhum tema adicional; abra caminhos variados dentro da lente"}\n\nO que ela não quer receber: ${avoid || "nenhuma restrição adicional"}`;
